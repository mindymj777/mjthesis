...
model_0 = torch.load(f'/home/pcdm/Desktop/mjpaper/cifar100/model_{model_algo}_0_92.pth')
...
def generate_union_model(split_mode):
    ...
    with open(f'/home/pcdm/Desktop/mjpaper/cifar100/loss/loss_indices_{split_mode[1]}.pkl', 'rb') as f:
        data = pickle.load(f)
        indexF = data['indexF']
        indexT = data['indexT']
   ...
   # model_decision=model_train(model_algo,train_dl,valid_dl,"decision",100,model_0=model_0,root=split_mode)
    model_decision = torch.load(f'/home/pcdm/Desktop/mjpaper/cifar100/loss/model_alexnet_decision_{split_mode}.pth')
    print("Load model decision OK.")

    #訓練模型T、F、decision
    # model_T=model_train(model_algo,Tdl,Tdl_v,"T",100,model_0=model_0,root=split_mode)
    model_T=torch.load(f'/home/pcdm/Desktop/mjpaper/cifar100/loss/model_alexnet_T_{split_mode}.pth')
    print("Load model_T OK.")

    #訓練模型T、F、decision
    # model_F=model_train(model_algo,Fdl,Fdl_v,"F",100,model_0=model_0,root=split_mode)
    model_F=torch.load(f'/home/pcdm/Desktop/mjpaper/cifar100/loss/model_alexnet_F_{split_mode}.pth')
    ...
    df_list=[]
    for union in range(5,20,5):
        union=round(union/10,2)
        if(split_mode[1] <1):
            union=round(union+split_mode[1],2)
        # if(split_mode[1]==1.):
        #     if union<=0.1:
        #         union=round(union+split_mode[1],2)
        #     else:
        #         union=round(union-0.1,2)
        #         union=round(split_mode[1]-union,2)
        elif(split_mode [1]>1):
            union=round(split_mode[1]-union,2)
        ...
        if(split_mode[0]=='classf1'):
            ...
            split=(split_mode[0],split_mode[1],union)
        ...
        #訓練模型T、F、decision
        if abs(len(indexT_u)-len(indexT))>200:
        ...
        #訓練模型T、F、decision
        if abs(len(indexF_u)-len(indexF))>200:
...
df.to_csv(f'output_ciar100_loss_{i}_union.csv', mode='a', header=write_header, index=False )
write_header = False
empty_df = pd.DataFrame([[]])
empty_df.to_csv(f'output_ciar100_loss_{i}_union.csv', mode='a', header=False, index=False)

write_header=True
combined_df.to_csv(f'output_ciar100_loss_{i}_union.csv', mode='a', header=write_header, index=False )
write_header = False
empty_df = pd.DataFrame([[]])
empty_df.to_csv(f'output_ciar100_loss_{i}_union.csv', mode='a', header=False, index=False)
